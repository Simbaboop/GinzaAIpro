...
2. **Governance Operating System controls** — accepted Canonical Governance Standards in `governance/standards/` and established governance-process artifacts define how governed work proceeds beneath the Constitution. They govern process and evidence but do not replace capability-specific ADRs, HCES contracts, or runtime semantics.
3. **Accepted decisions** — non-empty ADRs explicitly marked `Accepted` in established ADR locations, plus explicit human-approved canonical decisions recorded through `CHANGE_PROTOCOL.md` with durable resolution evidence.
4. **Explicit canonical architecture documents** — documents explicitly marked `Canonical` within their recorded architecture scope.
5. **Explicit canonical specifications** — versioned HCES, capability specifications, or equivalent specifications explicitly marked canonical or accepted within their stated scope.
6. **Approved architecture and product specifications** — architecture and `docs/` specifications that state normative rules but do not carry an explicit canonical status.
7. **Implementation-local contracts** — subsystem and feature README files governing their own directory scope.
8. **Planning and exploratory material** — roadmaps, future-evolution notes, discovery, research, generated starter READMEs, and empty placeholders.
